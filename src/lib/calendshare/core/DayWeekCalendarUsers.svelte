<script lang="ts">
	...
	import type { DayWeekCalendarUserData } from "../db/collections/DayWeekCalendars"
	import { loginAsGuest } from "$lib/stores/auth"
	import { user } from "$lib/stores/auth"
	import { popup, type PopupSettings } from "@skeletonlabs/skeleton"

	const { store: state, toggleVisibilityForUser } =
		getContext<DayWeekCalendarContext>("dayWeekCalendarState")

	let users: UserData[] = []
	$: if ($state.calendar) {
		fetchUsers($state.calendar.users).then((usersData) => (users = usersData))
	}

	export async function fetchUsers(users: Array<DayWeekCalendarUserData>) {
		const usersData: UserData[] = []
		for (const calendarUser of users) {
			const res = await fetch(`/api/users/${calendarUser.uid}`)
...
{:then loadedUsers}
	<ul class="text-left flex flex-col gap-1">
		{#each loadedUsers as loadedUser}
			<li
			...
			>
				...
				<div class="flex gap-1">
					<button
						on:click={() => handleToggleVisibilityForUser(loadedUser.uid)}
					...
					{#if !$user && loadedUser.email == "guest"}
						<button
							on:click={() => handleExistingGuestLogin(loadedUser.uid)}
							class="btn variant-filled-surface">Login</button
						>
					{/if}
					{#if $state.calendar?.ownerId == $user?.uid && loadedUser.uid != $user?.uid}
						...
						<div class="card p-4 variant-filled-primary z-100 w-40" data-popup="popupClick">
							...
							<button
								on:click={() => handleRemoveUser(loadedUser.uid)}
