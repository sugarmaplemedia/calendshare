<script lang="ts">
	...
	import { getToastStore, Table, tableMapperValues, type TableSource } from "@skeletonlabs/skeleton"
...
<div class="w-full flex items-center justify-center h-full">
	<div class="md:grid p-4 w-full md:grid-cols-3 gap-4 max-w-5xl">
		<form
			class="card w-full variant-ghost-surface p-4 mb-4 lg:mb-0 grid gap-4"
			method="post"
			action="?/update"
		...
		<div class="flex flex-col gap-8 card p-4 md:col-span-2 h-fit">
			<div class="flex justify-between">
				<h2 class="h2">All Calendshares</h2>
						...
						{#each calendshareRecords as record}
							<tr>
								...
								<td class="align-middle overflow-ellipsis overflow-hidden"
									>{record.calendshare.owner.firstName} {record.calendshare.owner.lastName}</td
								>
								<td>
									<button
										on:click={() => handleLeave(record.calendshareId)}
										class="btn variant-ghost-error py-0"
									>
										Leave
									</button>
								</td>
